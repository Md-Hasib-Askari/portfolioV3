...
export default function HeroSection({ className }: { className?: string }) {
    return (
        <section className={`${sectionStyle} justify-center items-center text-center px-4 ${className}`}>
            ...
            <p className="max-w-5xl mx-auto text-center mt-4 text-sm sm:text-base lg:text-lg text-muted-foreground px-4">
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-5 mt-6 px-4">
                <Button onClick={() => scrollToSection("projects")} size="default" className="p-2 px-3 rounded-lg flex justify-center gap-2 items-center space-x-2 bg-orange-500 hover:bg-orange-500/90 text-white text-sm sm:text-base">
                    Projects{' '}<Globe className="w-4 h-4 sm:w-5 sm:h-5" />
                </Button>

                <Button onClick={() => scrollToSection("about")} variant="default" size='default' className="bg-background text-foreground hover:text-muted-foreground border border-border flex items-center space-x-2 text-sm sm:text-base">
                    Find out more
                </Button>
            </div>
        </section>
